            ...
            if msg_type == "webrtc.offer":
                ...
                if chat:
                    if chat.chat_type == 'private':
                        ...
                        logger.info(
                            f"Call offer in chat {self.chat_id}. Notifying {len(others)} others: {others}")
                        ...
                        for uid in others:
                            logger.info(
                                f"Sending notify.call to user_notify_{uid}")
                            await self.channel_layer.group_send(
                                f'user_notify_{uid}',
                                {
                                    'type': 'notify.call',
                        ...
                        logger.warning(
                            f"Call offer received for non-private chat {self.chat_id}. Global notifications skipped.")
...
class NotifyConsumer(AsyncWebsocketConsumer):
    ...
    async def notify_call(self, event):
        logger.info(
            f"NotifyConsumer: Sending incoming.call to user {self.user.id} from {event.get('from_user_id')}")
        await self.send(text_data=json.dumps({
            'type': 'incoming.call',
