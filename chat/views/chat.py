...
def dashboard(request):
    ...
    for tweet in tweets_queryset[:20]:  # Limit to 20 most recent
        ...
        is_saved = SavedPost.objects.filter(tweet=tweet, user=request.user).exists()
...
def get_chat_messages(request, chat_id):
    ...
    for msg in messages_query:
        ...
        is_read = msg.read_receipts.exclude(user=msg.sender).exists() if msg.sender else False
...
def p2p_send_signal(request):
    ...
    try:
        ...
        signal_data = data.get('signal_data')  # Contains type, offer/answer/candidate, fileInfo
        ...
        logger.info(f"P2P signal stored: {signal_data.get('type', 'unknown')} from user {request.user.id} to user {target_user_id}")
...
def p2p_get_signals(request, chat_id):
    ...
    try:
        ...
        if signal_ids:
            P2PSignal.objects.filter(id__in=signal_ids).update(is_consumed=True)
            logger.info(f"P2P signals consumed by user {request.user.id}: {len(signal_ids)} signals")
